...
const demoVideo = "https://www.youtube.com/embed/3ht7I9-0A_k?autoplay=1&rel=0";
          ...
          <div className="space-y-4">
            <div className="space-y-2">
              {headlineLines.map((line, lineIndex) => (
                <div
                  key={line}
                  className="flex flex-wrap gap-x-3 overflow-hidden text-5xl font-bold leading-tight text-white sm:text-6xl xl:text-7xl"
                >
                  {line.split(" ").map((word, wordIndex) => (
                    <motion.span
                      ...
                      transition={{
                        delay: lineIndex * 0.16 + wordIndex * 0.08,
                        duration: 0.55,
                      }}
                    >
                      {word}
            ...
            >
              All-in-one GST invoicing, UPI payments, expense tracking and
              business reports - built for Indian businesses.
            </motion.p>
          </div>
          <motion.div
            animate={{ opacity: 1, y: 0 }}
            ...
            <div className="relative rounded-[28px] border border-white/10 bg-slate-950/80 p-5 light:border-slate-200 light:bg-white">
              <div className="flex items-center justify-between border-b border-white/10 pb-4 light:border-slate-200">
                <div>
                  ...
                  <p className="mt-2 text-2xl font-semibold text-white light:text-slate-950">
                    #INV-2024
                  </p>
                </div>
              ...
              <div className="grid gap-4 py-5 sm:grid-cols-2">
                <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-4 light:border-slate-200 light:bg-slate-50">
                  ...
                  <p className="mt-2 text-sm text-slate-400">
                    Collected via UPI QR in 1 click
                  </p>
                </div>
                <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-4 light:border-slate-200 light:bg-slate-50">
                  <p className="text-xs uppercase tracking-[0.2em] text-slate-500">
                    ...
                    >
                      <div
                        className={cn(
                          ...
                          index === 5
                            ? "shadow-[0_0_18px_rgba(6,182,212,0.35)]"
                            : "",
                        )}
      ...
      {openVideo && (
        ...
        >
          ...
          >
            <button
              onClick={() => setOpenVideo(false)}
              className="absolute -top-12 right-0 text-white text-4xl transition hover:text-red-500"
            >
              ×
            </button>

            <div className="overflow-hidden rounded-2xl shadow-2xl aspect-video">
              <iframe
                src={demoVideo}
                title="Invoicezy Demo"
